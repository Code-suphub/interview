...
关于 `ConcurrentHashMap` 的详细介绍，请看我写的这篇文章：[`ConcurrentHashMap` 源码分析](docs/java/collection/knowledge/concurrent-hash-map-source-code.md)。
...
关于 `CopyOnWriteArrayList` 的详细介绍，请看我写的这篇文章：[`CopyOnWriteArrayList` 源码分析](docs/java/collection/knowledge/copyonwritearraylist-source-code.md)。
