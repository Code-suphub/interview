...
我们在执行执行一条**“增删改”（即对数据修改）**语句的时候，虽然没有输入 begin 开启事务和 commit 提交事务，但是 MySQL 会**隐式开启事务**来执行“增删改”语句的，执行完就自动提交事务的，这样就保证了执行完“增删改”语句后，我们可以及时在数据库表看到“增删改”的结果了。
...
- undo-log 本身是会被删除清理的，例如 INSERT 操作，在事务提交之后就可以清除掉了；(因为MVCC不需要插入的数据链，**插入只有在当前事务提交之后的事务才会看见**，所以提交完这个insert的undo log 就可以被删除了)
...
- binlog 有 3 种格式类型，分别是 STATEMENT、ROW、MIXED，区别如下：
  ...
  - MIXED：包含了 STATEMENT 和 ROW 模式，它会根据不同的情况自动使用 ROW 模式和 STATEMENT 模式，通常是判定这条SQL语句是否会造成数据不一致；
