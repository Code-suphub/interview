...
注意：X型间隙锁和X型临键锁不冲突，如果一个事务获取(3,10]的临键锁，且(3,10]区间内只有10这一条记录，那么另一个事务获取(3,10)的间隙锁是没问题的。
...
这次实验环境的 **MySQL 版本是 8.0.26，隔离级别是「可重复读」**。（如果是读已提交，在进行读取操作的时候是不会加间隙锁的，只有记录锁，~~但是写操作会加间隙锁，不确定~~）
...
- **如果这里是 select * from user where id > 14 for update; 那么在id =15 的数据上加的也是next-key锁* ** 
...
所以，**当有一个事务持有二级索引的间隙锁 (22, 39) 时，插入 age = 22 或者 age = 39 记录的语句是否可以执行成功，关键还要考虑插入记录的主键值，*一定注意*：因为「二级索引值（age 列）+ 主键值（id 列）」才可以确定插入的位置，确定了插入位置后，就要看插入的位置的下一条记录是否有间隙锁，如果有间隙锁，就会发生阻塞，如果没有间隙锁，则可以插入成功**。
