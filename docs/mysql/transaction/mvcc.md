...
那么**在同时处理多个事务的时候，就可能出现脏写（dirty write）、脏读（dirty read）、不可重复读（non-repeatable read）、幻读（phantom read）的问题**。

接下来，通过举例子给大家说明，这些问题是如何发生的。

### 脏写

意思是说有两个事务，事务 A 和事务 B 同时在更新一条数据，事务 A 先把它更新为 A 值，事务 B 紧接着就把它更新为 B 值。（对于mysql数据库直接默认会解决掉，因为写锁是必须的，sql标准不允许不加写锁）。
...
- m_ids：指的是在创建 Read View 时，当前数据库中「活跃事务」的**事务 id 列表**，注意是一个列表，**“活跃事务”指的就是，启动了但还没提交的事务**。
  - 按理说通过字典实现查询会更快，特别是在有长事务导致min_trx_id和max_trx_id差距很大的时候，但是貌似大多说底层实现就是列表
