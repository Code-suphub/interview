...
所以，有一种服务器就专门保存了 `Web` 服务器域名与 `IP` 的对应关系，它就是 `DNS` 服务器。通过DNS（Domain Name System）域名管理系统进行**域名和 IP 地址的映射**
...
- 根 DNS 服务器（.），目前世界上已经有13组根服务器（600多台），我国境内目前仍没有根服务器。
- 顶级域 DNS 服务器，如`com`、`org`、`net`和`edu`等。国家也有自己的顶级域，如`uk`、`fr`和`ca`。
...
浏览器会先看自身有没有对这个域名的缓存，如果有，就直接返回，如果没有，就去问操作系统，操作系统也会去看自己的缓存，如果有，就直接返回，如果没有，再去 hosts 文件看，也没有，才会去问「本地 DNS 服务器」。本地DNS服务也会缓存很多的顶级域名服务器。减少对根服务器的压力

当然这个缓存也是有过期时间的，一段时间这个资源没有被访问过就会删除，减少资源占用压力。
...
除了做流量控制以外，TCP 还会做**拥塞控制**，对于真正的通路堵车不堵车，它无能为力，唯一能做的就是控制自己，也即控制发送的速度。
...
至此，网络包的报文生成，在HTTP报文的基础上加上TCP头部。
